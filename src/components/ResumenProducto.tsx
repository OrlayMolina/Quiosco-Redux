...
import { selectPedido, setProducto, setModal, selectModal, setPedido } from "../features/quioscoSlice";

export default function ResumenProducto({producto}: {producto: ProductoProps}): JSX.Element  {

    const dispatch = useDispatch();
    const pedido = useSelector(selectPedido);
    const modalCantidad = useSelector(selectModal);
    const { id, nombre, precio, cantidad } = producto;
    
    const handleEditarCantidad = (id: number) => {
      const productoActualizar = pedido.filter(producto => producto.id === id)[0];
       dispatch(setProducto(productoActualizar));
       dispatch(setModal(!modalCantidad));
    }

    const handleProductoEliminado = (id: number) => {
      const pedidoActualizar = pedido.filter(producto => producto.id !== id);
       dispatch(setPedido(pedidoActualizar));
        ...
        <div className="shadow space-y-1 p-4 bg-white">
          <div className="space-y-2">
            ...
            <p className="text-lg text-gray-700">
              Subtotal: {formatearDinero(precio * (cantidad ?? 0))}
            </p>
          </div>
